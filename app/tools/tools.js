...
function loadGUIStrings() {
    ...
    return new Promise(function (resolve, reject) {
        ...
        var stringFiles = ["guiStrings.json", "guiStringsSpeech.json"];
        ...
        stringFiles.forEach(fileName => {
            ...
            jsonFile.readText()
                .then(function (content) {
                    try {
                        global.guiStrings.push(JSON.parse(content));
 ...
 * @param {*} stringFile 
 */
function bindGuiStrings(bindObject, stringFile) {
    
    global.guiStrings.forEach(fileName => {
 ...
 */
function parseHealthRecord(healthRecord) {
    var record = [];
    record.timestamp = healthRecord[0];
    record.general = healthRecord[1];
    record.dia = healthRecord[2];
    record.eat = healthRecord[3];
    record.pain = healthRecord[4];
    record.oral = healthRecord[5];
    record.appetite = healthRecord[6];
    record.pnp = healthRecord[7];
    record.fatigue = healthRecord[8];
    record.activity = healthRecord[9];
    record.weight = healthRecord[10];
    record.comments = healthRecord[11];
    record.medication = healthRecord[12];
    record.hypertension = healthRecord[13];
    record.eczema = healthRecord[14];
    record.liver = healthRecord[15];
    record.vision = healthRecord[16];
    record.muscle = healthRecord[17];
    record.joint = healthRecord[18];
    record.raceheart = healthRecord[19];
...
function parseHealthRecordList(healthRecord) {
    var record = [];
    record = {
        "timestamp": healthRecord[0],
        "general": healthRecord[1],
        "dia": healthRecord[2],
        "eat": healthRecord[3],
        "pain": healthRecord[4],
        "oral": healthRecord[5],
        "appetite": healthRecord[6],
        "pnp": healthRecord[7],
        "fatigue":healthRecord[8],
        "activity": healthRecord[9],
        "weight": healthRecord[10],
        "comments": healthRecord[11],
        "medication": healthRecord[12],
        "hypertension": healthRecord[13],
        "eczema": healthRecord[14],
        "liver": healthRecord[15],
        "vision": healthRecord[16],
        "muscle": healthRecord[17],
        "joint": healthRecord[18],
        "breath": healthRecord[19],
        "exercise": healthRecord[20],
        "raceheart": healthRecord[21]
    };
...
exports.parseHealthRecord = parseHealthRecord;
